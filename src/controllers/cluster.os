...
Функция list() Экспорт
	...
	ПараметрыЗамера = ЗамерыВремени.НачатьЗамер(ЗапросHTTP.Путь, ЗапросHTTP.СтрокаЗапроса, "cluster", "get");
	...
	Результат = ОбщегоНазначения.ДанныеВJSON(ПодключенияКАгентам.Кластеры(Поля, Фильтр, Истина));
...
Функция get() Экспорт
	...
	ПараметрыЗамера = ЗамерыВремени.НачатьЗамер(ЗапросHTTP.Путь, ЗапросHTTP.СтрокаЗапроса, "cluster", "list");
	...
	Данные = ПодключенияКАгентам.Кластер(АдресСервера, ПортСервера, Поля, Истина);
	
	Если ЗначениеЗаполнено(ИмяПараметра) Тогда
		Если Данные = Неопределено Тогда
			ЗначениеПараметра = ПодключенияКАгентам.ПустойОбъектКластера("cluster", Поля)[ИмяПараметра];
		Иначе
