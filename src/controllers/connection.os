...
Функция list() Экспорт
	...
	Результат = ОбщегоНазначения.ДанныеВJSON(Подключение.Соединения(Поля, Фильтр, Первые, Истина));
...
Функция get() Экспорт
	...
	Данные = Подключение.Соединение(ИБ, Соединение, Поля, Истина);
	
	Если ЗначениеЗаполнено(ИмяПараметра) Тогда
		Если Данные = Неопределено Тогда
			ЗначениеПараметра = Подключение.ПустойОбъектКластера("connection", Поля)[ИмяПараметра];
		Иначе
