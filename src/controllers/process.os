...
Функция list() Экспорт
	...
	Результат = ОбщегоНазначения.ДанныеВJSON(Подключение.Процессы(Поля, Фильтр, Первые, Истина));
...
Функция get() Экспорт
	...
	Данные = Подключение.Процесс(АдресСервера, ПортСервера, Поля, Истина);
	
	Если ЗначениеЗаполнено(ИмяПараметра) Тогда
		Если Данные = Неопределено Тогда
			ЗначениеПараметра = Подключение.ПустойОбъектКластера("process", Поля)[ИмяПараметра];
		Иначе
