...
Функция list() Экспорт
	...
	Результат = ОбщегоНазначения.ДанныеВJSON(Подключение.Сеансы(Поля, Фильтр, Первые, Истина));
...
Функция get() Экспорт
	...
	Данные = Подключение.Сеанс(ИБ, Сеанс, Поля, Истина);
	
	Если ЗначениеЗаполнено(ИмяПараметра) Тогда
		Если Данные = Неопределено Тогда
			ЗначениеПараметра = Подключение.ПустойОбъектКластера("session", Поля)[ИмяПараметра];
		Иначе
