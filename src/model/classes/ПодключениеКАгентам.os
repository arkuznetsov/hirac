...
Функция ОписанияОбъектовКластера(Знач ТипОбъекта, Знач Обновить = Ложь,
	...
	Для Каждого ТекОписаниеОбъекта Из КэшОписанийОбъектов Цикл
		...
		Если ТипОбъекта = ТипыОбъектовКластера1С.ИнформационныеБазы Тогда
			ДополнитьОписаниеИнформационнойБазыДаннымиСУБД(ОписаниеОбъектаДляВывода);
		КонецЕсли;
...
Процедура ПроверитьОбновитьАвторизацию(ОбъектКластера)

	Попытка
		ПараметрыАвторизации = ОбъектКластера.ПараметрыАвторизации();
	Исключение
		ТекстОшибки = СтрШаблон("Ошибка получения параметров авторизации объекта ""%1""", ТипЗнч(ОбъектКластера));
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;

	Если ТипЗнч(ПараметрыАвторизации) = Тип("Структура") Тогда
		Если ПараметрыАвторизации.Свойство("Администратор") И ЗначениеЗаполнено(ПараметрыАвторизации.Администратор) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗнч(ОбъектКластера) = Тип("УправлениеКластером1С") Тогда
		НовыеПараметрыАвторизации = ПараметрыПодключения.Агенты[ОбъектКластера.СтрокаПодключения()];
	ИначеЕсли ТипЗнч(ОбъектКластера) = Тип("Кластер") Тогда
		АдресКластера = СтрШаблон("%1:%2", ВРег(ОбъектКластера.АдресСервера()), ОбъектКластера.ПортСервера());
		НовыеПараметрыАвторизации = ПараметрыПодключения.Кластеры[АдресКластера];
	Иначе
		Возврат;
	КонецЕсли;

	ОбъектКластера.УстановитьАдминистратора(НовыеПараметрыАвторизации.Администратор, НовыеПараметрыАвторизации.Пароль);

КонецПроцедуры // ПроверитьОбновитьАвторизацию()
...
Функция ПолныеОписанияОбъектовАгента(Знач ОписаниеАгента, Знач ТипОбъекта)
	...
	ПроверитьОбновитьАвторизацию(ОписаниеАгента.Агент);
	...
	СписокКластеров = Новый ФиксированныйМассив(ОписаниеАгента.Кластеры);
...
Функция ПолныеОписанияОбъектовКластера(Знач Кластер, Знач ТипОбъекта, Знач Агент)
	...
	Попытка
		ПроверитьОбновитьАвторизацию(Кластер);
		СписокОбъектовКластера = Кластер.Получить(ТипОбъекта).Список(, , Истина);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение СтрШаблон("Ошибка получения объектов ""%1"" кластера ""%2"" (%3) :%4%5",
		                            ТипОбъекта,
		                            Кластер.Имя(),
		                            Кластер.Ид(),
		                            Символы.ПС,
...
Процедура РасширитьПолеОбъектаКластера(ПолноеОписаниеОбъекта, ИмяПоля, ТипОбъекта, ОбъектКластера)
	...
	Если ТипОбъекта = ТипыОбъектовКластера1С.Серверы
	   И ВРег(ИмяПоля) = "SERVER" Тогда
		...
		ПортСервера  = ОбъектКластера.Получить("ПортГлавногоМенеджераКластера");
	...
	Если ТипОбъекта = ТипыОбъектовКластера1С.ИнформационныеБазы
	   И ВРег(ИмяПоля) = "INFOBASE" Тогда
		ПолноеОписаниеОбъекта.Вставить("infobase-label", ОбъектКластера.Получить("Имя"));
	 КонецЕсли;
...
Процедура ДополнитьОписаниеИнформационнойБазыПоПараметрамСУБД(ОписаниеИБ)
	...
	ПолеОшибки = "db-error";
	Если ОписаниеИБ[ПолеОшибки] = Неопределено Тогда
		ОписаниеИБ.Вставить(ПолеОшибки, Неопределено);
	КонецЕсли;
...
Процедура ДополнитьОписаниеИнформационнойБазыДаннымиСУБД(ОписаниеИБ)
	...
	ЗанимаемоеМесто = ОписаниеЗанимаемогоМестаБД();
	ЗанимаемоеМестоДанныеСУБД = ОписаниеЗанимаемогоМестаБД();

	Попытка
		ЗанимаемоеМестоДанныеСУБД = СтруктураХраненияИБ.ЗанимаемоеМесто();
	Исключение
		ТекстОшибки = СтрШаблон("Ошибка получения информации о занимаемом месте БД ""%1/%2"":%3%4",
		                        ОписаниеИБ["db-server"],
		                        ОписаниеИБ["db-name"],
		                        Символы.ПС,
	...
	ЗаполнитьЗначенияСвойств(ЗанимаемоеМесто, ЗанимаемоеМестоДанныеСУБД);
	...
	ОписаниеИБ.Вставить("db-log-total"        , ЗанимаемоеМесто.ЖурналВсего);
	ОписаниеИБ.Вставить("db-log-used"         , ЗанимаемоеМесто.ЖурналИспользуется);
...
Процедура ОбновитьКэшОписанийОбъектовКластеров(Знач ТипОбъекта, Знач Обновить = Ложь)
	...
	ВремОписанияОбъектовКластеров = Новый Соответствие();
	ОшибкаПолученияОписаний = Ложь;
	...
	Для Каждого ТекАгент Из Агенты Цикл
		...
		Исключение
			ТекстОшибки = СтрШаблон("Ошибка получения описаний объектов ""%1"" от агента ""%2"":%3%4",
			                        ТипОбъекта,
			                        ТекАгент.Ключ,
			                        Символы.ПС,
			                        ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Сообщить(ТекстОшибки, СтатусСообщения.ОченьВажное);
			ОшибкаПолученияОписаний = Истина;
			Продолжить;
		КонецПопытки;
		
		Для Каждого ТекОписаниеОбъекта Из ОписанияОбъектовАгента Цикл
			ИдОбъекта = ТекОписаниеОбъекта.Значение[ТипОбъекта];
			ДобавлятьОбъект = ДобавлятьОбъектИнформационнойБазы(ТекОписаниеОбъекта.Значение, ТипОбъекта);

			Если НЕ (ДобавлятьОбъект И ВремОписанияОбъектовКластеров[ИдОбъекта] = Неопределено) Тогда
				Продолжить;
			КонецЕсли;

			ВремОписанияОбъектовКластеров.Вставить(ИдОбъекта, ТекОписаниеОбъекта.Значение);
		КонецЦикла;

	КонецЦикла;

	Если ЗначениеЗаполнено(ВремОписанияОбъектовКластеров) Тогда
		ОписанияОбъектовКластеров = КэшОписанийОбъектовКластера(ТипОбъекта, Истина);
		Для Каждого ТекОписание Из ВремОписанияОбъектовКластеров Цикл
			ОписанияОбъектовКластеров.Вставить(ТекОписание.Ключ, ТекОписание.Значение);
		КонецЦикла;
	КонецЕсли;

КонецПроцедуры // ОбновитьКэшОписанийОбъектовКластеров()

// Функция - для объектов информационных баз, сеансов и соединений проверяет необходимость добавления
// в соответствии с настройками вывода информации только об информационных базах, для которых заданы настройки
//
// Параметры:
//   ОписаниеОбъекта          - Соответствие        - описание объекта кластера 1С
//   ТипОбъекта               - Строка              - тип объектов кластера к получению
//
// Возвращаемое значение:
//   Булево    - Истина - объект нужно добавлять
//
Функция ДобавлятьОбъектИнформационнойБазы(ОписаниеОбъекта, ТипОбъекта)

	Если ТипОбъекта = ТипыОбъектовКластера1С.ИнформационныеБазы Тогда
		ПараметрыСУБД = Настройки.ПараметрыСУБДИБ(ОписаниеОбъекта["_thisObject"], Истина);
		Если ПараметрыСУБД = Неопределено Тогда
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли ТипОбъекта = ТипыОбъектовКластера1С.Сеансы
		  ИЛИ ТипОбъекта = ТипыОбъектовКластера1С.Соединения Тогда
		ИдИБ = ОписаниеОбъекта["infobase"];
		ПараметрыСУБД = Настройки.ПараметрыСУБДИБ(ИдИБ, Истина);
		Если ПараметрыСУБД = Неопределено И НЕ ОбщегоНазначения.ЭтоПустойGUID(ИдИБ) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
КонецФункции // ДобавлятьОбъектИнформационнойБазы()
...
// Функция - получает структуру описания занимаемого места базой данных
//
// Возвращаемое значение:
//   Соответствие    - структуру описания занимаемого места базой данных
//
Функция ОписаниеЗанимаемогоМестаБД();

	ЗанимаемоеМесто = Новый Структура();
	ЗанимаемоеМесто.Вставить("РазмерБазы");
	ЗанимаемоеМесто.Вставить("Свободно");
	ЗанимаемоеМесто.Вставить("Зарезервировано");
	ЗанимаемоеМесто.Вставить("Данные");
	ЗанимаемоеМесто.Вставить("Индексы");
	ЗанимаемоеМесто.Вставить("НеИспользуется");
	ЗанимаемоеМесто.Вставить("ЖурналВсего");
	ЗанимаемоеМесто.Вставить("ЖурналИспользуется");
	ЗанимаемоеМесто.Вставить("КоличествоТаблиц");

	Возврат ЗанимаемоеМесто;

КонецФункции // ОписаниеЗанимаемогоМестаБД()
...
Функция ПолучитьКластерыАгента(ОписаниеАгента)
	...
	ОбнаруженныеКластеры = Новый Массив();
	Если Настройки.ТолькоВыбранныеКластеры() Тогда
		Для Каждого ТекКластер Из КластерыАгента Цикл
			...
			Если НЕ Настройки.ПараметрыПодключения().Кластеры[ВРег(МеткаКластера)] = Неопределено Тогда
				ОбнаруженныеКластеры.Добавить(ТекКластер);
			КонецЕсли;
		КонецЦикла;
	Иначе
		ОбнаруженныеКластеры = КластерыАгента;
	КонецЕсли;

	ОписаниеАгента.Кластеры = Новый ФиксированныйМассив(ОбнаруженныеКластеры);
...
Функция ЭтоПолеОсновногоОписанияОбъекта(ТипОбъекта, ИмяПоля)
	...
	Для Каждого ТекПоле Из ПоляОбъекта Цикл
		...
		ИначеЕсли ВРег(ТекПоле.ИмяРАК) = "INFOBASE" Тогда
			...
			ПоляОсновнойИнформации.Вставить("DB-LOG-TOTAL"        , Истина);
			ПоляОсновнойИнформации.Вставить("DB-LOG-USED"         , Истина);
