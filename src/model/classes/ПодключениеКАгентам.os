...
Функция ОписанияОбъектовКластера(Знач ТипОбъекта, Знач Обновить = Ложь,
	...
	Для Каждого ТекОписаниеОбъекта Из КэшОписанийОбъектов Цикл
		...
		Если НЕ ОбщегоНазначения.ОбъектСоответствуетФильтру(ОписаниеОбъекта, Фильтр) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПолучитьОписаниеОбъектаИзКэша(ИдОбъекта, ТипОбъекта)
	...
	КонецЕсли;
	Если ОписаниеОбъекта = Неопределено И ОбщегоНазначения.ЭтоПустойGUID(ИдОбъекта) Тогда
...
Функция ЭтоПолеОсновнойИнформацииОбъекта(ТипОбъекта, ИмяПоля)
...
КонецФункции // ЭтоПолеОсновнойИнформацииОбъекта()
...
Функция НужноДобавлятьПоле(ТипОбъекта, ДобавляемыеПоля, ИмяПоля)
	...
	Если ТипОбъекта = ТипыОбъектовКластера().ИнформационныеБазы
	 ИЛИ ТипОбъекта = ТипыОбъектовКластера().Соединения Тогда
		Если НЕ ДобавляемыеПоля.Найти("_SUMMARY") = Неопределено И ЭтоПолеОсновнойИнформацииОбъекта(ТипОбъекта, ИмяПоля) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
...
Функция ОписаниеОбъектаКластера(ТипОбъекта,
	...
	Для Каждого ТекПолеОписанияОбъекта Из ПолноеОписаниеОбъекта Цикл
		...
		Если ТипОбъекта = ТипыОбъектовКластера().ИнформационныеБазы
			...
			ЭтоПолеРасширенойИнформацииОбъекта = НЕ ЭтоПолеОсновнойИнформацииОбъекта(ТипОбъекта, ИмяПоля);

			// Для полей ИБ и соединений проверяем наличие полного описания
			Если ЭтоПолеРасширенойИнформацииОбъекта
			И ПолноеОписаниеОтсутствует Тогда
			...
			КластерОбъекта  = ПолноеОписаниеОбъекта["_clusterObject"];
			Если ТипОбъекта = ТипыОбъектовКластера().ИнформационныеБазы
			И ЭтоПолеРасширенойИнформацииОбъекта Тогда
				ИдОбъекта      = ПолноеОписаниеОбъекта["infobase"];
				ОбъектКластера = КластерОбъекта.ИнформационныеБазы().Получить(ИдОбъекта);
			ИначеЕсли ТипОбъекта = ТипыОбъектовКластера().Соединения
			И ЭтоПолеРасширенойИнформацииОбъекта Тогда
				ИдОбъекта      = ПолноеОписаниеОбъекта["connection"];
				ОбъектКластера = КластерОбъекта.Соединения().Получить(ИдОбъекта);
			КонецЕсли;

			Если ЭтоПолеРасширенойИнформацииОбъекта
			И НЕ ОбъектКластера.ПолноеОписание() Тогда

				ОбъектКластера.ОбновитьДанные(1);

				Если НЕ ОбъектКластера.ПолноеОписание() Тогда
					ПоляПолногоОписания = ПоляПолногоОписания + ?(ПоляПолногоОписания = "", "", ", ") + ИмяПоля;
					ПолноеОписаниеОтсутствует = Истина;
					Продолжить;
				КонецЕсли;

				Если ТипОбъекта = ТипыОбъектовКластера().ИнформационныеБазы Тогда
					ПолноеОписаниеОбъектаКластера = КластерОбъекта.ИнформационныеБазы().Получить(ИдОбъекта, , Истина);
				ИначеЕсли ТипОбъекта = ТипыОбъектовКластера().Соединения Тогда
					ПолноеОписаниеОбъектаКластера = КластерОбъекта.Соединения().Получить(ИдОбъекта, , Истина);
				КонецЕсли;
				ПолноеОписаниеОбъектаКластера.Вставить("_clusterObject", КластерОбъекта);

				ЗначениеПоля = ОбъектКластера.Получить(ИмяПоля);

				ОбновленныеОписания.Вставить(ИдОбъекта, ПолноеОписаниеОбъектаКластера);
			КонецЕсли;
		КонецЕсли;
	...
	// Если было запрошено поле из полного описания ИБ или соединения , но не смогли его получить,
	// то добавляем поле с информациет об ошибке
	Если ЗначениеЗаполнено(ПоляПолногоОписания) И ПолноеОписаниеОтсутствует Тогда
		ТекстОшибки =
			СтрШаблон("Не удалось получить поля ""%1"" полного описания объекта %2, проверьте настройки авторизации!",
			          ПоляПолногоОписания,
			          ТипОбъекта);
...
Процедура ИнициализироватьКластерыАгента(УправлениеКластером)
	...
	Пока КластерыАгента = Неопределено И ПопыткаПолучения <= КоличествоПопыток Цикл
		КластерыАгента = УправлениеКластером.Кластеры().Список(, 0);
