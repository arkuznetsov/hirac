...
Процедура ДополнитьОписаниеИнформационнойБазыДаннымиСУБД(ПолноеОписаниеОбъекта)
	...
	Если НЕ (ЗначениеЗаполнено(ПолноеОписаниеОбъекта["dbms"])
	       И ЗначениеЗаполнено(ПолноеОписаниеОбъекта["db-server"])
	       И ЗначениеЗаполнено(ПолноеОписаниеОбъекта["db-name"])
	       И ЗначениеЗаполнено(ПараметрыСУБД.СУБД_Пользователь)) Тогда
		ТекстОшибки = СтрШаблон("Для информационной базы ""%1"", указаны не все параметры подключения",
		                        ПолноеОписаниеОбъекта["name"]);
		ПолноеОписаниеОбъекта.Вставить("db-error", ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Попытка
		БазаСуществует = РаботаССУБД.БазаСуществует(ПолноеОписаниеОбъекта["db-name"]);
	Исключение
		ТекстОшибки = СтрШаблон("Ошибка проверки существования базы данных ""%1"", на сервере ""%2"" для информационной базы ""%3"":%4%5",
	...
	ПолноеОписаниеОбъекта.Вставить("db-table-count"      , ЗанимаемоеМесто.КоличествоТаблиц);

КонецПроцедуры // ДополнитьОписаниеИнформационнойБазыДаннымиСУБД()

Процедура ЗаполнитьОписаниеТаблицБазыДанныхИБ(ПолноеОписаниеОбъекта)
...
КонецПроцедуры // ЗаполнитьОписаниеТаблицБазыДанныхИБ()
...
Процедура ОбновитьКэшОписанийОбъектовКластеров(Знач ТипОбъекта, Знач Обновить = Ложь)
	...
	Если ВРег(ТипОбъекта) = ВРег(ТипыОбъектовКластера().Таблицы) Тогда
		Если НЕ ЗначениеЗаполнено(ОписанияИнформационныхБаз) Тогда
			ОбновитьКэшОписанийОбъектовКластеров(ТипыОбъектовКластера().ИнформационныеБазы);
		КонецЕсли;
		Для Каждого ТекОписаниеИБ Из ОписанияИнформационныхБаз Цикл
			ЗаполнитьОписаниеТаблицБазыДанныхИБ(ТекОписаниеИБ.Значение);
		КонецЦикла;
		Возврат;
	КонецЕсли;
