...
Процедура ОбновитьКластеры(Знач Поля = "_all") Экспорт
...
Функция Кластеры(Знач Поля = "_all", Знач Обновить = Ложь) Экспорт
		...
		Для Каждого ТекЭлемент Из ПоляКластера Цикл
			...
			Если ТекЭлемент.Ключ = "cluster" Тогда
				ЗначениеЭлемента = СтрШаблон("%1:%2",
				                             ТекКластер[ПоляКластера["host"].Имя],
				                             ТекКластер[ПоляКластера["port"].Имя]);
			КонецЕсли;
