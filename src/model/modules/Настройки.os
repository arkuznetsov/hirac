...
Перем ТолькоВыбранныеКластеры;             // - Булево    - Истина - данные будут получаться только с кластеров,
...
//
// Параметры:
//   ИБ_Ид                        - Строка    - идентификатор ИБ в кластере или объект ИБ
//                                  ИнформационнаяБаза
//   НеопределеноЕслиНеНайдено    - Булево    - Истина - если настройки базы не обнаружены, возвращать "Неопределено"
//                                              Ложь - возвращать новую структуру настроек базы
//
// Возвращаемое значение:
//   Структура, Неопределено                    - параметры подключения к СУБД информационной базы
...
//
Функция ПараметрыСУБДИБ(Знач ИБ_Ид, НеопределеноЕслиНеНайдено = Ложь) Экспорт
...
// Функция - возвращает флаг получения данных с кластеров, указанных в настройках
//
// Возвращаемое значение:
//   Булево    -  Истина - данные будут получаться только с кластеров, указанных в настройках
//
Функция ТолькоВыбранныеКластеры() Экспорт

	ПеречитатьИзмененияНастроек();

	Возврат ТолькоВыбранныеКластеры;

КонецФункции // ТолькоВыбранныеКластеры()
...
Процедура ЗаполнитьПараметры(НастройкиДляУстановки)
	...
	ТолькоВыбранныеКластеры            = ПолучитьЗначениеНастройки("selectedClustersOnly"    , НастройкиДляУстановки);
	...
	КонецЕсли;
	Если ТолькоВыбранныеКластеры = Неопределено Тогда
		ТолькоВыбранныеКластеры = Ложь;
	КонецЕсли;
...
Функция СтруктураПараметровКластера()
	...
	СтруктураПараметров.Вставить("Администратор"    , "");
	СтруктураПараметров.Вставить("Пароль"           , "");
	СтруктураПараметров.Вставить("ТолькоУказанныеИБ", Ложь);
...
Функция ПараметрыКластера(МеткаКластера, НастройкиДляУстановки)
	...
	ТолькоВыбранныеИБПоУмолчанию = ПолучитьЗначениеНастройки("selectedInfobasesOnly", ПараметрыПоУмолчанию, Ложь);
	ТолькоВыбранныеИБ            = ПолучитьЗначениеНастройки("selectedInfobasesOnly",
	                                                         ПараметрыПоИмени,
	                                                         ТолькоВыбранныеИБПоУмолчанию);
	ПараметрыКластера.Вставить("ТолькоВыбранныеИБ", ТолькоВыбранныеИБ);
...
Процедура УстановитьПараметрыКластера(Кластер) Экспорт
	...
	КонецЕсли;
	ТолькоВыбранныеИБ = Ложь;
	Если НЕ ПараметрыКластера.Свойство("ТолькоВыбранныеИБ", ТолькоВыбранныеИБ) Тогда
		ПараметрыКластераПоУмолчанию.Свойство("ТолькоВыбранныеИБ", ТолькоВыбранныеИБ);
	КонецЕсли;
	...
	Для Каждого ТекИБ Из СписокИБ Цикл
		Если ТолькоВыбранныеИБ И ПараметрыКластера.ИБ[ВРег(ТекИБ.Имя())] = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
// Процедура - проверяет наличие изменений в файле настроек
// и если файл изменился выполняет чтение актуальных настроек из файла
//
