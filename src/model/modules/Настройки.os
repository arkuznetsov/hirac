...
Процедура УстановитьПараметрыИБ(ИБ, ПараметрыКластера)
	...
	КонецЕсли;
	Если НЕ ПараметрыИБ.Свойство("СУБД_Тип", СУБД_Тип) Тогда
		ПараметрыИБПоУмолчанию.Свойство("СУБД_Тип", СУБД_Тип);
	КонецЕсли;
	Если НЕ ПараметрыИБ.Свойство("СУБД_Сервер", СУБД_Сервер) Тогда
		ПараметрыИБПоУмолчанию.Свойство("СУБД_Сервер", СУБД_Сервер);
	КонецЕсли;
	Если НЕ ПараметрыИБ.Свойство("СУБД_Пользователь", СУБД_Пользователь) Тогда
		ПараметрыИБПоУмолчанию.Свойство("СУБД_Пользователь", СУБД_Пользователь);
	КонецЕсли;
	Если НЕ ПараметрыИБ.Свойство("СУБД_Пароль", СУБД_Пароль) Тогда
		ПараметрыИБПоУмолчанию.Свойство("СУБД_Пароль", СУБД_Пароль);
	КонецЕсли;
	Если НЕ ПараметрыИБ.Свойство("СУБД_База", СУБД_База) Тогда
		ПараметрыИБПоУмолчанию.Свойство("СУБД_База", СУБД_База);
	КонецЕсли;

	ИБ.УстановитьАдминистратора(Администратор, Пароль);
	
	ИБ.УстановитьПараметрыСУБД(СУБД_Тип, СУБД_Сервер, СУБД_Пользователь, СУБД_Пароль, СУБД_База);
