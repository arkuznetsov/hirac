...
Функция Список(Знач ТипОбъектов = Неопределено) Экспорт
	...
	Для Каждого ТекРаздел Из ПараметрыСчетчиков() Цикл
		...
		Счетчики.Вставить(ТекРаздел.Ключ, Новый Структура());
...
Функция АгрегироватьЗначенияСчетчика(ЗначенияСчетчика, Знач РезультирующиеИзмерения = "", Знач АгрегатнаяФункция = "count")
	...
	Если ТипЗнч(РезультирующиеИзмерения) = Тип("Строка") Тогда
		...
		Для й = 0 По Измерения.ВГраница() Цикл
			Измерения[й] = ВРег(СокрЛП(Измерения[й]));
		КонецЦикла;
	...
	Если НЕ Измерения.Найти(ВРег("_all")) = Неопределено Тогда
		Возврат ЗначенияСчетчика;
	ИначеЕсли НЕ Измерения.Найти(ВРег("_no")) = Неопределено Тогда
		Измерения = Новый Массив();
	КонецЕсли;
	...
	Для Каждого ТекЗначение Из ЗначенияСчетчика Цикл
		...
		Для Каждого ТекИзмерение Из Измерения Цикл
			Если НЕ ТекЗначение.Свойство(ТекИзмерение) Тогда
				Продолжить;
			КонецЕсли;
	...
	Для Каждого ТекЭлемент Из ГруппыЗначений Цикл
		...
		АгрегированноеЗначение = Новый Структура();
		Для Каждого ТекИзмерение Из Измерения Цикл
...
Функция ЗначенияСчетчиков(Знач Объекты, Знач ПараметрыСчетчиковОбъекта, Знач Счетчик = "", Знач Фильтр = Неопределено)
	...
	Для Каждого ТекСчетчик Из ОписаниеСчетчиков Цикл
		...
		Счетчики.Вставить(ИмяСчетчика, Новый Структура());
		...
		Для Каждого ТекОбъект Из Объекты Цикл
			...
			Если ЗначениеЗаполнено(Фильтр) И ТипЗнч(Фильтр) = Тип("Структура") Тогда
				СоответствуетФильтру = Истина;
				Для Каждого ТекЭлементФильтра Из Фильтр Цикл
					Если НЕ ЗначениеСчетчика.Свойство(ТекЭлементФильтра.Ключ) Тогда
						СоответствуетФильтру = Ложь;
						Прервать;
					КонецЕсли;
					Если НЕ ВРег(ЗначениеСчетчика[ТекЭлементФильтра.Ключ]) = ВРег(ТекЭлементФильтра.Значение) Тогда
						СоответствуетФильтру = Ложь;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				Если НЕ СоответствуетФильтру Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
		...
		Для Каждого ТекЗначение Из ТекСчетчик.Значение["values"] Цикл
			...
			Для Каждого ТекИзмерение Из ТекЗначение Цикл
				...
				ЗначенияИзмеренийСтрокой = ЗначенияИзмеренийСтрокой +
				                           ?(ЗначенияИзмеренийСтрокой = "", "", ",") +
				                           СтрШаблон("%1=""%2""", ТекИзмерение.Ключ, ЗначениеИзмерения);
			КонецЦикла;
